...
router.post("/create", upload.fields([{ name: 'certificateFile', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    const query = `
      ...
      (course, student, type, academic_year, semester, start_date, end_date, exam_date, mark, certificate_url, certificate_path, approval_status, certificate_type, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '1')
    `;
