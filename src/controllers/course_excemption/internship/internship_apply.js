...
router.post("/internshipApply", upload.fields([
  ...
  try {
    ...
    const query = `
      ...
      (student, semester,  mode, industry, start_date, end_date, duration, stipend, amount, type, report_path, certificate_path, elective)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    const query1 = `
      INSERT INTO ce_intern_registered
      (student, semester,  mode, industry, start_date, end_date, duration, stipend, amount, type, report_path, certificate_path)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
