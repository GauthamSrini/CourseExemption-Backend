...
exports.post_reject_intern = async (req, res) => {
    const { remark, id ,user_id} = req.body;

    try {
        //Fetch the current approval_status
        const queryFetchStatus = 'SELECT approval_status FROM ce_intern_registered WHERE id = ?';
        ...
        } else if (user_id === 7 && currentApprovalStatus === 1) {
            isAllowedToUpdate = true;
        } else if (user_id === 8 && currentApprovalStatus === 2) {
            isAllowedToUpdate = true;
        } else if (user_id === 4 && currentApprovalStatus === 3) {
            isAllowedToUpdate = true;
        }
