...
exports.processExcelFileHonorMinor = async (req, res) => {
  ...
  WHERE student = ? AND course_code = ? AND course_name = ?`;
    ...
    if (existingStudent.length === 0) { // If student_id does not exist
      const sqlInsert = `INSERT INTO ce_honor_minor_mappings (student, semester, mode_of_exemption, course_code, course_name)
      VALUES (?, ?, ?, ?, ?)`
      let code = data[0]
      let name = data[1]
      let student = data[2]
      ...
      try {
        await post_query_database(sqlInsert,[student,semester,modeOfExp,code,name]);
