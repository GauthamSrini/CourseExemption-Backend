...
exports.post_reject_addon_honor_minor = async (req, res) => {
    const { remark, id,user_id } = req.body;

    try {
        //Fetch the current approval_status
        const queryFetchStatus = 'SELECT approval_status FROM ce_addon_honor_minor_registered WHERE id = ?';
        ...
        if (user_id === 2 && currentApprovalStatus === 0) {
            isAllowedToUpdate = true;
        } else if (user_id === 3 && currentApprovalStatus === 1) {
            isAllowedToUpdate = true;
        ...
        const queryUpdate = `
            UPDATE ce_addon_honor_minor_registered 
            SET approval_status = -1, remarks = ?, rejected_by = ? 
            WHERE id = ?
        `;
