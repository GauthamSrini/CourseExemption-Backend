...
exports.get_rejected_addon_honor_minor = async (req, res) => {
    const { approval_status, type, rejected_by } = req.query;
    ...
    try {
        ...
        const query = `
            ...
            WHERE a.approval_status = ? AND a.rejected_by >= ? AND a.status = '1' ${modeCondition}
        `;

        const students = await get_query_database(query, [approval_status,rejected_by]);
