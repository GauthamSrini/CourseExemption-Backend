...
exports.get_approved_one_credit = async (req, res) => {
    ...
    WHERE oc.approval_status >= ? AND oc.status = '1'
`;
    try {
        const oneCredit_approvals = await get_query_database(query,[approval_status]);
        res.status(200).json(oneCredit_approvals);
    } catch (err) {
