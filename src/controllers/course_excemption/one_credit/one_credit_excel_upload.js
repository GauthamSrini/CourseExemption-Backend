...
exports.processExcelFileOneCredit = async (req, res) => {
  ...
  async function processRow(row) {
    ...
    if (existingStudent.length === 0) { // If student_id does not exist
      const sqlInsert = `INSERT INTO ce_onecredit_mappings (course_code, name, student, semester) VALUES (?, ?, ?, ?)`
      try {
        await post_query_database(sqlInsert,[code,name,student,semester]);
      ...
      const sqlInsert1 = `UPDATE ce_onecredit_mappings SET name = ?, semester = ? WHERE course_code = ? AND student = ?`
    try {
  ...
  try {
    ...
    const uniqueCoursesQuery = `SELECT DISTINCT course_code, name FROM ce_onecredit_mappings`;
    ...
    for (const course of uniqueCourses) {
      ...
      if (existingCourse.length === 0) {
        const insertQuery = `INSERT INTO ce_onecredit_courselist (course_code, course_name) VALUES (?, ?)`;
        try {
          await post_query_database(insertQuery, [course.course_code, course.name]);
    ...
    res.status(200).json({ message: 'Added All the Details in Database Courses Added : ', added, updated, skip });
  } catch (error) {
