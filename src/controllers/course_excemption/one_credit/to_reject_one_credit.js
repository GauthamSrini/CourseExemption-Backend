...
exports.post_reject_one_credit = async (req, res) => {
    const { remark, id ,user_id} = req.body;

    try {
        // Step 1: Fetch the current approval_status
        const queryFetchStatus = 'SELECT approval_status FROM ce_onecredit_registered WHERE id = ?';
        ...
        const queryUpdate = `
            UPDATE ce_onecredit_registered 
