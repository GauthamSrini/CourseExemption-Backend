...
exports.post_Excemption = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO ce_onecredit_registered (course_1, course_2, course_3, student)
      VALUES (?, ?, ?, ?)
    `;
    await post_query_database(query, [selectedCourses[0], selectedCourses[1], selectedCourses[2], studentId]);
